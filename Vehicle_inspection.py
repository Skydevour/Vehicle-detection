#-*- coding:utf-8 -*- 
# @Time : 2021/2/10 11:16 
# @Author : 万志杨
# @File : Vehicle_inspection.py 
# @Software: PyCharm
...
# 使用TensorFlow实现yolo_non_max_suppression()。TensorFlow有两个内置函数可用于实现非极大值抑制（因此你实际上不需要使用你的iou()实现方法）：
...
# 实现yolo_eval()，获取YOLO编码的输出，并使用得分阈值和NMS过滤框。你只需要了解最后一个实现细节。有几种表示框的方法，例如通过它们的角或中点和高度/宽度。YOLO使用以下函数（我们已提供）在不同时间内在这几种类格式之间进行转换：
...
#
# 车辆识别数据集包含720x1280图像，我们已将其预处理为608x608的图像。
...
yolo_model.summary()
# yolo_model 的输出是（m，19，19，5，85）张量，需要通过非特殊的处理和转换。以下单元格为你完成了该操作。
